...
typedef struct axes_parm
{
	...
	char	auto_x, auto_y, y_label_int;
	char	uniform_scale, center;
	char	log_x, log_y;
	char	x_small_log, x_medium_log;
	char	y_small_log, y_medium_log;
	char	check_y_cursor;
...
/* ****** pltcon.c ********/
void condata(DrawStruct *d, float label_size, Matrx *a, char auto_ci,
			float *ci, float cmin, float cmax);
void contour2(DrawStruct *d, float label_size, int n1, int nx, int ny, float *a,
			double *xg, double *yg, float exc, char auto_ci,
...
/* ****** drawps.c ********/
void gdrawInitPS(FILE *fp, char do_color, char do_portrait);
