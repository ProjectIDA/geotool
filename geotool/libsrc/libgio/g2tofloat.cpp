...
not used
static long gain_range[4] = {1 << 13, 1 << 15, 1 << 17, 1 << 30};
 */

#define TOP_TWO	(unsigned char)0xc0	/* top two bits */
#define BOT	(unsigned char)0x3f	/* bottom 6 bits */
 ...
 * @param from array with gain ranged data
 * @param to degained data
 * @param num the number of entries
 */
void
g2tofloat(unsigned char *from, void *to, int outtype, int num)
{
	long	value;
	float	*fdata;
	int	*idata;
	...
	if (outtype == FLOAT_DATA)
	{
	    ...
	    for (fdata += (num - 1), from += (num-1) * BYTES_PER_G2;num-- ;
	     	     from -= BYTES_PER_G2, --fdata)
	    {
		value = ((from[0] & BOT) << 8) | from[1];
		*fdata = (value - BIAS) << gain_value[(TOP_TWO & from[0]) >> 6];
	    }
	}
	else if (outtype == INT_DATA)
	{
	    ...
	    for (idata += (num - 1), from += (num-1) * BYTES_PER_G2;num-- ;
	     	     from -= BYTES_PER_G2, --idata)
	    {
		value = ((from[0] & BOT) << 8) | from[1];
		*idata = (value - BIAS) << gain_value[(TOP_TWO & from[0]) >> 6];
	    }
	}
}
