/** \file DataValues.cpp
 *  \brief Defines class DataValues.
 *  \author Ivan Henson
 */
#include "config.h"
#include <iostream>
...
DataValues::DataValues(const string &name, Component *parent,
	WaveformPlot *wave_plot) : FormDialog(name, parent, false)
{
    wp = wave_plot;
    createInterface();
}

void DataValues::createInterface(void)
{
    Arg args[20];
    int n;
    ...
    XtSetArg(args[n], XtNcolumns, 11); n++;
    XtSetArg(args[n], XtNvisibleRows, 10); n++;
    XtSetArg(args[n], XtNwidth, 850); n++;
    const char *column_labels[] = {
	"sta", "chan", "time", "epoch time", "time - start", "time - origin",
	"counts", "counts*calib", "seg", "index in seg", "index from beg"};
...
//    XtSetArg(args[n], XtNdisplayMenuButton, True); n++;
    XtSetArg(args[n], XtNtableTitle, "Data Values"); n++;
    table = new Table("table", this, args, n);
/*
    table->setAttributes(
"sta,%s,chan,%s,time,%t,time - start,%.2f,time - origin,%.2f,count,%d,counts*calib,%.4f");
*/
    ...
    int col_align[11] = { LEFT_JUSTIFY, LEFT_JUSTIFY, RIGHT_JUSTIFY,
		...
		RIGHT_JUSTIFY, RIGHT_JUSTIFY, RIGHT_JUSTIFY, RIGHT_JUSTIFY};
    table->setAlignment(11, col_align);

    remove_cursor = false;
}

DataValues::~DataValues(void)
{
}

void DataValues::actionPerformed(ActionEvent *action_event)
{
...
void DataValues::setVisible(bool visible)
{
...
void DataValues::displayDataCursor(void)
{
   if(!wp) return;
...
void DataValues::removeDataCursor(void)
{
    if(!wp) return;
...
ParseCmd DataValues::parseCmd(const string &cmd, string &msg)
{
    ParseCmd ret;
...
ParseVar DataValues::parseVar(const string &name, string &value)
{
    ParseVar ret;
...
void DataValues::parseHelp(const char *prefix)
{
