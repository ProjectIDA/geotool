...
/*
 * NAME
 *	small_circle_cross_pts -- Determine crossing points of 2 small circles.

 * FILE
 *	small_circle_cross_pts.c
 ...
 * DIAGNOSTICS
 *	If no valid crossing points exist, return error.

 * NOTES
 *	Non-Naperian trigonometry must be used due to the sphericity of the
 *	Earth.

 * SEE ALSO
 *	Great circle crossing routine, azimuth_cross_pt().
...
int
small_circle_cross_pts (double olat1, double olon1, double olat2, double olon2, double rsmall, double rlarge, 
		    double *xlat1, double *xlon1,double *xlat2,double *xlon2)
...

{
	...
	dist_azimuth (olat1, olon1, olat2, olon2, &stadist, &azi, &baz, 0);
	...
	lat_lon (olat2, olon2, rlarge, azi, xlat1, xlon1);

	azi = azi - 2.0*alpha;
	if (fabs(azi) > 180.0)
		azi = SIGN((360.0-fabs(azi)), azi);
	lat_lon (olat2, olon2, rlarge, azi, xlat2, xlon2);
